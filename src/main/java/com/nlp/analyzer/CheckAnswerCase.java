...
public class CheckAnswerCase {
	...
	protected int getGrade() {
		Tools.getInstance().logger.println("getGrade :::: TEACHER :" + teacher_ans);
		Tools.getInstance().logger.println("getGrade :::: STUDENT :" + student_ans);
		...
		if (teacher_ans.getMap().containsKey(student_ans.getContent())) {
			Tools.getInstance().logger.println("using map: " + student_ans.getContent());
			Tools.getInstance().logger.println("grade: " + teacher_ans.getMap().get(student_ans.getContent()));
		...
		if (firstGrade==teacher_ans.getGrade()) {
			...
			Tools.getInstance().logger.println("finished grading: " + student_ans.getContent());
			Tools.getInstance().logger.println("grade: " + firstGrade);
			return firstGrade;
		}
		...
		Tools.getInstance().logger.println("finished grading: " + student_ans.getContent());
		Tools.getInstance().logger.println("grade: " + grade);
	...
	private int equalSentences(Answer constant, Answer compareTo, boolean OppMode) {
		if(OppMode) Tools.getInstance().logger.println("ANALYZER :::: COMPARING TEACHER TO STUDENT");
		else Tools.getInstance().logger.println("ANALYZER :::: COMPARING STUDENT TO TEACHER");
		...
		for(Token consta : constant.getAnalyzed_ans().getTokensList()) {
			...
			if (Arrays.asList(Tools.getInstance().del).contains(consta.getPartOfSpeech().getTag())) {
				Tools.getInstance().logger.println("equalSentences :::: we have insignificant word: " + consta.getText().getContent());
				continue;
			}
			
			for(Token comp : compareTo.getAnalyzed_ans().getTokensList()) {
				
				if (Arrays.asList(Tools.getInstance().del).contains(comp.getPartOfSpeech().getTag())) {
					Tools.getInstance().logger.println("equalSentences :::: we have insignificant word: " + comp.getText().getContent());
					continue;
				}
				...
				Tools.getInstance().logger.println("equalSentences :::: checking " + teacher.getText().getContent() + " >> COMPARE TO >> " + student.getText().getContent());
				
				if (!set.contains(comp)) {
					if (equalNodes(constant, compareTo, teacher, student, OppMode)) {
						set.add(comp);
						found=true;
						Tools.getInstance().logger.println("equalSentences1 :::: finished path check, equal: " + teacher.getText().getContent() + " " + student.getText().getContent());
						break;
					}else {
						Tools.getInstance().logger.println("equalSentences1 :::: One path check, NOT equal: " + teacher.getText().getContent() + " " + student.getText().getContent());
					}
				}else {
					Tools.getInstance().logger.println("equalSentences1 :::: One path check, Set contains already: " + comp.getText().getContent());
				}
			}
			if (!found) {
				Tools.getInstance().logger.println("equalSentences1 :::: finished path check WITH FOUND=FALSE! for constant word: " + consta.getText().getContent());
				Tools.getInstance().logger.println("equalSentences1 :::: finishedGrade: " + finishedGrade);
				Tools.getInstance().logger.println("equalSentences1 :::: grade: " + Math.max(finishedGrade, Tools.getInstance().MINGRADE));
				return Math.max(finishedGrade, Tools.getInstance().MINGRADE);
			}
			found=false;
		}
		//the grade is maxGrade-mistakes*10(can be changed)
		Tools.getInstance().logger.println("equalSentences1 :::: finished path check WITH FOUND=TRUE: " + constant.getContent() + " " + compareTo.getContent());
		Tools.getInstance().logger.println("equalSentences1 :::: equalset = : " + equalSet);
		Tools.getInstance().logger.println("equalSentences1 :::: finishedGrade: " + finishedGrade);
		Tools.getInstance().logger.println("equalSentences1 :::: teacher_ans.getAnswerWords(): " + teacher_ans.getAnswerWords());
		Tools.getInstance().logger.println("equalSentences1 :::: equalSet.size(): " + equalSet.size());
...
//		int finalGrade = Math.max(finishedGrade, teacher_ans.getGrade()-(Math.abs(teacher_ans.getAnswerWords()-equalSet.size())*ApiHolder.getInstance().REDUCE));
		...
		finalGrade = Math.min(finalGrade, Tools.getInstance().MAXGRADE);
		if (student_ans.getWriter().equals("COMPUTER")) return finalGrade-Tools.getInstance().COMP;
	...
	private boolean equalNodes (Answer constant, Answer compareTo, Token teacher, Token student, boolean OppMode) {
		if (Arrays.asList(Tools.getInstance().del).contains(teacher.getPartOfSpeech().getTag()) || Arrays.asList(Tools.getInstance().del).contains(student.getPartOfSpeech().getTag())) {
			Tools.getInstance().logger.println("equalNodes :::: not intresting: " + teacher.getText().getContent() + " + " + student.getText().getContent()); 
			return true;
		}

		if (compare(teacher, student)) {
			Tools.getInstance().logger.println("equalNodes :::: equal tokens: " + teacher.getText().getContent() + " + " + student.getText().getContent()); 
			equalSet.add(new EqualTokens(teacher, student));
			Tools.getInstance().logger.println(equalSet);
			...
			if (t_father.equals(teacher) || s_father.equals(student)) {
				Tools.getInstance().logger.println("equalNodes :::: no father... returning true. bye"); 
				return true;
			}else{
				Tools.getInstance().logger.println("equalNodes :::: there are fathers: " + t_father.getText().getContent() + " + " + s_father.getText().getContent()); 
			...
			for(Token trick : compareTo.getAnalyzed_ans().getTokensList()) {
				...
				Tools.getInstance().logger.println("equalNodes :::: trick loop var: " + var.getText().getContent() + " +to tricked>>: " + trick.getText().getContent()); 
				if(trick.getDependencyEdge().getHeadTokenIndex()==index1 && trick.getDependencyEdge().getLabel()!=Label.ROOT) {
					Tools.getInstance().logger.println("equalNodes :::: trick try var: " + var.getText().getContent() + " +to tricked>>: " + trick.getText().getContent()); 
					if (OppMode) {
			...
			Tools.getInstance().logger.println("equalNodes :::: not equal tokens: " + teacher.getText().getContent() + " + " + student.getText().getContent()); 
			return false;
		}
	}
	...
	private boolean compare (Token teacher, Token student)  {
		if (checkTokens(teacher,student)) {
			Tools.getInstance().logger.println("compare :::: passed checkTokens V "+ teacher.getText().getContent() + " + " + student.getText().getContent()); 
				if (checkParts(teacher,student)) {
					Tools.getInstance().logger.println("compare :::: passed checkParts V "+ teacher.getText().getContent() + " + " + student.getText().getContent()); 
					if (checkRelationToParent(teacher,student)) {
						Tools.getInstance().logger.println("compare :::: passed checkRelationToParent V "+ teacher.getText().getContent() + " + " + student.getText().getContent()); 
						return true;
					}
				}
		}else if (specialCases1(teacher, student)) {
			Tools.getInstance().logger.println("compare :::: special case1 V "+ teacher.getText().getContent() + " + " + student.getText().getContent()); 
			return true;
		}
		else return false;
		return false;
	...
	private boolean checkTokens(Token teacher, Token student) {
		if (teacher.getText().getContent().equals(student.getText().getContent()) || teacher.getLemma().equals(student.getLemma())) return true;
			...
			for(String sgg : Tools.getInstance().getSpelling(student.getText().getContent())) {
				if(sgg.toLowerCase().equals(student.getText().getContent().toLowerCase())) continue;
				if (teacher.getText().getContent().equals(sgg) || LevenshteinDistance.computeLevenshteinDistance(teacher.getText().getContent(), sgg)<Tools.getInstance().LEVENSHTEIN) {
//					System.out.println(teacher.getText().getContent() + "  " + student.getText().getContent());
					...
					if (!teacher_ans.getMap().containsKey(new_s1)) {
						Tools.getInstance().logger.println("checkTokens :::: spelling fixed: " + student.getText().getContent() + " >> " + sgg);
						CheckAnswerCase newCheck = new CheckAnswerCase(Tools.getInstance().factory.createAnswer(new_s1, 0, "COMPUTER").build(), teacher_ans);
						Tools.getInstance().logger.println("----starting new check-----");
						finishedGrade = Math.max(finishedGrade, newCheck.getGrade());
						Tools.getInstance().logger.println("----finished new check-----");
					}else finishedGrade = teacher_ans.getMap().get(new_s1)-Tools.getInstance().COMP;// we already computed this sentence
				}
			}
			for(Entailment ent : Tools.getInstance().getEntailmentList(student_ans.getContent())) {
				if (ent.getMatchedWords().get(0).getToken().equals(student.getText().getContent()) && teacher.getText().getContent().equals(ent.getEntailedWords().get(0)) && ent.getContextScore()>Tools.getInstance().MEANING){	
					String[] new_s = student_ans.getContent().split(" ");
					new_s[student_ans.getAnalyzed_ans().getTokensList().indexOf(student)] = ent.getEntailedWords().get(0);
					String new_s1 = StringUtils.join(new_s, " ");
					
					if (!teacher_ans.getMap().containsKey(new_s1)) {
						Tools.getInstance().logger.println("checkTokens :::: meaning fixed: " + student.getText().getContent() + " >> " + ent.getEntailedWords().get(0));
						CheckAnswerCase newCheck = new CheckAnswerCase(Tools.getInstance().factory.createAnswer(new_s1, 0, "COMPUTER").build(), teacher_ans);
						Tools.getInstance().logger.println("----starting new check-----");
						finishedGrade = Math.max(finishedGrade, newCheck.getGrade());
						Tools.getInstance().logger.println("----finished new check-----");
					}else finishedGrade = teacher_ans.getMap().get(new_s1)-Tools.getInstance().COMP;
				}
			}
		}
		return false;
